...
#  =========================== Code to plot assignment data - Question 2 ================================= #
# Constants
chrOutput_file <- "plot2.png"
chrDate_format <- "%d/%m/%Y"
# Save current wd
chrInit_wd <- getwd()
# Now obtain the data
...
plot((as.POSIXct(strptime(dtHpc_selected$Date,format=chrDate_format,tz="GMT"))+as.ITime(dtHpc_selected$Time)),dtHpc_selected$Global_active_power,xlab="",ylab="Global Active Power (kilowatts)",type="n")
# and add the line
