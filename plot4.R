...
#  =========================== Code to plot assignment data - Question 4 ================================= #
plot1 <- function() {
    hist(dtHpc_selected$Global_active_power,col="red", main="Global Active Power",xlab="Global Active Power (kilowatts)")
}
plot2 <- function() {
    # Now build the plot canvas
    plot((as.POSIXct(strptime(dtHpc_selected$Date,format=chrDate_format,tz="GMT"))+as.ITime(dtHpc_selected$Time)),dtHpc_selected$Global_active_power,xlab="",ylab="Global Active Power",type="n")
    # and add the line
    lines(as.POSIXct(strptime(dtHpc_selected$Date,format=chrDate_format,tz="GMT"))+as.ITime(dtHpc_selected$Time),dtHpc_selected$Global_active_power,type="l")
}
plot3 <- function() {
    ...
    legend("topright",legend=c("Sub_metering_1","Sub_metering_2", "Sub_metering_2"),col=c("black","red","blue"),lty=c(1,1,1),bty="n")
}
plot4 <- function(){
    plot((as.POSIXct(strptime(dtHpc_selected$Date,format=chrDate_format,tz="GMT"))+as.ITime(dtHpc_selected$Time)),dtHpc_selected$Voltage,xlab="datetime",ylab="Voltage",type="n")
    # and add the lines
    lines(as.POSIXct(strptime(dtHpc_selected$Date,format=chrDate_format,tz="GMT"))+as.ITime(dtHpc_selected$Time),dtHpc_selected$Voltage,type="l")
}
plot5 <- function() {
    plot((as.POSIXct(strptime(dtHpc_selected$Date,format=chrDate_format,tz="GMT"))+as.ITime(dtHpc_selected$Time)),dtHpc_selected$Global_reactive_power,xlab="datetime",ylab="Global_reactive_power",type="n")
    # and add the lines
    lines(as.POSIXct(strptime(dtHpc_selected$Date,format=chrDate_format,tz="GMT"))+as.ITime(dtHpc_selected$Time),dtHpc_selected$Global_reactive_power,type="l")
}
# Constants
chrOutput_file <- "plot4.png"
chrDate_format <- "%d/%m/%Y"
# Save current wd
chrInit_wd <- getwd()
# Now obtain the data
...
png(chrOutput_file, width = 480, height = 480, units = "px", pointsize = 10, bg = "white")
# Set global parameters, graphic grid in particular
par(mfrow=c(2,2))
# Build the various graphs
plot2()
plot4()
plot3()
plot5()
